 ...
 */
faim_export int aim_chat_ban(aim_session_t *sess, aim_conn_t *conn, const char *sn)
{
	aim_frame_t *fr;
	aim_snacid_t snacid;

	if (!sess || !conn || !sn || !strlen(sn))
		return 0;

	if (!(fr = aim_tx_new(sess, conn, AIM_FRAMETYPE_FLAP, 0x02, 11+strlen(sn))))
		return -ENOMEM;

	snacid = aim_cachesnac(sess, 0x000e, 0x0024, 0x0000, NULL, 0);
 ...
 */
faim_export int aim_chat_unban(aim_session_t *sess, aim_conn_t *conn, const char *sn)
{
	aim_frame_t *fr;
	aim_snacid_t snacid;
	...
	snacid = aim_cachesnac(sess, 0x000e, 0x0025, 0x0000, NULL, 0);
